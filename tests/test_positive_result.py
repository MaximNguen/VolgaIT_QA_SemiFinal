...
class TestFormsPage:
    ...
    def test_required_field(self, browser, first_name, last_name, phone_number, username, mail, password):
        ...
        assert first_name_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.first_name_find().text}"
        assert last_name_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.last_name_find().text}"
        ...
        assert phone_number_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.phone_number_find().text}"
        assert username_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.username_find().text}"
        assert email_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.email_find().text}"
        assert password_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.password_find().text}"
        assert confirm_password_field_status == True, f"Ожидалось, что поле будет пустый после подтверждения, но в поле осталось текст {self.form_page.password_confirm_find().text}"
